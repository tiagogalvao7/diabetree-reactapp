...
const DataEntryScreen = () => {
    ...
    const loadReadings = async () => {
        try {
            // Fetch readings directly from your API
            ...
            if (!response.ok) {
                ...
                setRecentReadings([]); // Clear readings on error
                return;
            }
            ...
            // Set the recent readings, now we'll slice to get only the last 3 in the render
            setRecentReadings(data); 

        } catch (e) {
            console.error("Failed to load glucose readings from API:", e);
            Alert.alert("Error", "Could not load past readings from API.");
            setRecentReadings([]); // Clear readings on error
        }
    };
    ...
    const confirmSaveReading = async (value: number) => {
        const newReading: GlucoseReading = {
            id: Date.now().toString(), // Unique ID for the new reading
            value: value,
            timestamp: new Date().toISOString(),
        };

        try {
            // Send the new reading to your API
            const response = await fetch(`${API_BASE_URL}/glucoseReadings`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error(`HTTP error! status: ${response.status}, response: ${errorText}`);
                Alert.alert("Save Error", "Could not save reading to the API.");
                return;
            }
        ...
        Alert.alert(
            ...
            [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            // Delete the reading from your API
                            ...
                            if (!response.ok) {
                                const errorText = await response.text();
                                console.error(`HTTP error! status: ${response.status}, response: ${errorText}`);
                                Alert.alert("Delete Error", "Could not delete reading from the API.");
                                return;
                            }

                            loadReadings(); // Reload the list after deletion from API
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            // Fetch all reading IDs to delete them one by one from API
                            ...
                            // Delete each reading from the API
                            ...
                            setGlucoseValue('');
                            setRecentReadings([]); // Clear local state
                            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        ...
        <ScrollView contentContainerStyle={styles.scrollViewContent}>
            <View style={styles.container}>
                ...
                {recentReadings.length === 0 ? (
                    ...
                    recentReadings.slice(0, 3).map((reading) => ( // Display only the last 3 readings
